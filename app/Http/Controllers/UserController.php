...
use App\Models\ProfessorConfiguracao; // Importante para o Reajuste
    ...
    public function update(Request $request, User $usuario)
    {
        try {
            ...
            $request->validate([
                ...
                'perfil'    => 'required|in:admin,professor,aluno,socio',
                'matricula' => 'nullable|string|max:255', // Opcional, sem min (pode estar vazio)
                'cpf'       => 'nullable|unique:users,cpf,' . $usuario->id,
                'empresas'  => 'required|array|min:1', // Obrigatório escolher pelo menos uma empresa
            ], [
            ...
            return DB::transaction(function () use ($request, $usuario, $cpfLimpo) {
                ...
                $usuario->email = strtolower($request->email); // Minúscula
                $usuario->perfil = $request->perfil;
                $usuario->matricula = $request->matricula ? ucwords(strtolower($request->matricula)) : null; // Capitaliza se preenchido
   ...
   public function reajuste(Request $request, $id)
{
    ...
    // 2. Gravação no Banco
    // Importante: Usamos o ID do usuário que veio da URL
    \App\Models\ProfessorConfiguracao::create([
        'user_id'              => $id,
